			...
			if (i < 6) {
				...
				allGears.push(
					<div key={'gear' + i} className='charGear'>
						...
						<div className='gearInfo'>
							<h4>{curGear.name}</h4>
							{bonusEffect.map((effect, index) => (
								<p key={index}>{effect?.replace('[', '').replace(']', '').replace('Node', '')}</p>
							))}
							{engravingEffect.map((effect, index) => (
								<p key={index}>{effect?.replace('[', '').replace(']', '').replace('Node', '')}</p>
							))}
						</div>
					</div>
	...
	return (
		<div onClick={checkStatus} className={'charCard ' + (statsActive ? '' : 'collapsed')}>
				...
				<div className='charInfo'>
					...
					{advCharData ? (
						<h4 className='collapsible'>
							Full Stats & Gear<span className={'carrot ' + (statsActive ? 'open' : 'closed')}></span>
						</h4>
					) : (
						''
					)}
				...
				{editChar ? (
					<div className='editCharacterContainer'>
						<img
							...
							alt='refresh icon'
							className='updateCharBtn'
							onClick={() => {
								updateChar();
							}}></img>
						<button
							className='editCharBtn'
							onClick={() => {
					...
					<div className='applicantContainer'>
						...
						<div className='appBtns'>
							<button className='appDeclineBtn' value={char.id} onClick={decline}>
								Decline
							</button>
							<button className='appApproveBtn' value={char.id} onClick={approve}>
								Approve
							</button>
						</div>
					</div>
			...
			<div className='fullCharInfo'>
				<div className='charStats'>{allStats}</div>

				<div className='charGears'>{allGears}</div>
			</div>
		</div>
	);
}
