				...
				return (
					<React.Fragment key={char.id}>
						...
						{char?.joined
							...
							.map((group, index) => {
								return (
									<div key={group.id} id={group.id} className={group.id % 4 === 0 ? 'groupPreview bgTree' : group.id % 3 === 0 ? 'groupPreview bgGiant' : group.id % 2 === 0 ? 'groupPreview bgCity' : 'groupPreview bgSky'} onClick={handleGroupClick} style={{ animation: `fade-up 0.5s both ${index / 4 + 1.75}s` }}>
													...
													{group.member_char.map((character) => {
														return (
															<span key={character.id}>
																{character.groupmember.is_owner ? <>🟡</> : <></>} {character.char_name} ({character.item_lvl}){' '}
															</span>
														);
													})}
												</p>
										...
										<div className='groupPreviewColumnRight'>
											...
											<button className='previewApplyBtn' value={group.id}>
												View Details
											</button>
										</div>
									</div>
