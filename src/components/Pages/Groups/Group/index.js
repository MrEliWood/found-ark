...
function Group({ user, sendNoti, setBackground }) {
				...
				return (
					<div className='applicationSection'>
						<div className='appSectionRow'>
							...
							<button value={targetCharId} onClick={cancelApp}>
								Leave Group
							</button>
						</div>
					</div>
				...
				return (
					<div className='applicationSection'>
						<div className='appSectionRow'>
							...
							<button className='appSectionBtn' value={targetCharId} onClick={cancelApp}>
								Withdraw
							</button>
						</div>
					</div>
				...
				return (
					<div className='applicationSection'>
						<div className='appSectionRow'>
							...
							<button className='appSectionBtn' onClick={apply}>
								Apply
							</button>
						</div>
					</div>
	...
	const displayDiscord = () => {
		if (isGroupMember) {
			return (
				<>
					<a href={discordInfo[0].length > 10 ? discordInfo[0] : defaultDiscord[0]}>
						{discordInfo[0].length > 10 ? discordInfo[0] : defaultDiscord[0]} {discordInfo[0].length > 10 ? '' : <span className='defaultSpan'>(default link)</span>}
					</a>
				</>
			);
		}
						...
						<div className='allCharacters'>
							{groupMembers?.map((char) => (
								<div key={char.id} className={isGroupMember ? 'wrapCharCard groupMemberCard' : 'groupMemberCard'}>
									...
									{group?.creator?.owner_id === user?.id && char?.owner_id != user?.id && (
										<button id='kickMemberBtn' value={char?.id} onClick={decline}>
											Kick {char?.char_name}
										</button>
									)}
								</div>
					...
					{isGroupMember && (
						<div className='discordWidget'>
							{discordInfo?.length == 3 ? (
								<>
									<WidgetBot server={discordInfo[1]} channel={discordInfo[2]} height='50vh' width='100vw' />
								</>
							) : (
								<>
									<WidgetBot server={defaultDiscord[1]} channel={defaultDiscord[2]} height='50vh' width='100vw' />
								</>
							)}
						</div>
					)}
